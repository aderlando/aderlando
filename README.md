## Olá! Eu sou Aderlando 👋

**Estudante de Engenharia de Software e Técnico em Redes, FTTH/GPON **

Sou apaixonado por resolver 2 problemas: 
1. Levar internet de qualidade com redes de fibra óptica
2. Ensinar tecnologia de forma simples para quem tem mais de 60 anos

 🚀 Sobre mim
- 📍 João Pessoa - PB, Brasil
- 🔭 Trabalhando em: Projeto de Rede FTTH GPON
- 🌱 Estudando: Pytton, Node.js,
- 💬 Pergunte-me sobre: FTTH, GPON, Automação com Python, Didática para Idosos
- ⚡ Fun fact: Crio apostilas de informática com ilustrações para a melhor idade

🛠️ Tech Stack
`Pytton` `GPON` `FTTH` `Linux` `OTDR` `Git` `Canva`


📌 Projetos em Destaque
- [**Projeto FTTH GPON 10km**](#) - Rede econômica para 1000 domicílios

📫 Me encontre
- LinkedIn: [linkedin.com/in/aderlando](https://www.linkedin.com/in/aderlando-oliveira-a730b47b/)
- Email: aderlando-oliveira@hotmail.com

"Ensinar é conectar. Conectar é transformar."
